...
class Token:
    def __init__(self, tid: int, index: int, span_start: int, span_end: int, phrase: str, vocab_id: int, char_start: int, char_end: int):
...
class Entity:
    ...
    def as_tuple_token(self):
        span_token_list = []
        for tuple_t in self._tokens:
            span_token_list.append(tuple_t[0].index)
            span_token_list.append(tuple_t[-1].index+1)
        span_token_list.append(self._entity_type)
        return tuple(span_token_list)
...
class DiscontinuousDataset(TorchDataset):
    TRAIN_MODE = 'train'
    EVAL_MODE = 'eval'
    ...
    def __init__(self, label, rel_types, entity_types):
        ...
        self._mode = DiscontinuousDataset.TRAIN_MODE
    ...
    def create_token(self, idx, span_start, span_end, phrase, inx, char_start, char_end) -> Token:
        token = Token(self._tid, idx, span_start, span_end, phrase, inx, char_start, char_end)
    ...
    def create_entity(self, entity_type, tokens, phrases) -> Entity:
        mention = Entity(self._eid, entity_type, tokens, phrases)
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == DiscontinuousDataset.TRAIN_MODE:
            return sampling.create_train_sample_discontinuous(doc)
        else:
            return sampling.create_eval_sample_discontinuous(doc)
