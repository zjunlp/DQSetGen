...
class Criterion(nn.Module):
    ...
    def loss_boundary(self, outputs, targets, indices, num_spans):
        ...
        """
        if targets['spans'].shape[-1] > 2:  # two or three
            return self.loss_boundary_discontinuous(outputs, targets, indices, num_spans)
    ...
    def loss_boundary_discontinuous(self, outputs, targets, indices, num_spans):
        """Boundary loss (negative log likelihood loss)
        targets dicts must contain the key "spans"
        """
        assert 'pred_boundaries' in outputs
        ...
        n_span = target_spans.shape[-1] // 2 # two or three? target_spans.shape是4,6，除以2得出有几个span
        loss_boundary = 0.
        for i in range(n_span):  # 第i个span
            cur_idx = (idx[0], idx[1]*n_span+i) # 不同span的边界位置是不同的, idx[1]中的数值实际指的是第几个set
            src_spans_left = outputs['pred_boundaries'][0][cur_idx]
            src_spans_right = outputs['pred_boundaries'][1][cur_idx]

            src_spans_left_logp = torch.log(1e-25 + src_spans_left)
            src_spans_right_logp = torch.log(1e-25 + src_spans_right)

            left_nll_loss = F.nll_loss(src_spans_left_logp, target_spans[:,i*2], reduction='none')
            right_nll_loss = F.nll_loss(src_spans_right_logp, target_spans[:, i*2+1], reduction='none')

            loss_boundary += left_nll_loss + right_nll_loss

        losses = {}
        losses['loss_boundary'] = loss_boundary.sum() / (num_spans*n_span)
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            if num_queries != outputs["pred_boundaries"][0].shape[1]:
                return self.forward_discontinuous(outputs, targets)

            # We flatten to compute the cost matrices in a batch
            out_prob = outputs["pred_logits"].flatten(0, 1).softmax(dim=-1) # c, n_class

            entity_left = outputs["pred_boundaries"][0].flatten(0, 1)   # c, seq_len
            entity_right = outputs["pred_boundaries"][1].flatten(0, 1)  # c, seq_len
    ...
    def forward_discontinuous(self, outputs, targets):
        """ Performs the matching
        ...
        """
        with torch.no_grad():
            bs, num_queries = outputs["pred_logits"].shape[:2]              # num_q是有几个set

            # We flatten to compute the cost matrices in a batch
            out_prob = outputs["pred_logits"].flatten(0, 1).softmax(dim=-1) # c, n_class
            
            n_left_right = outputs["pred_boundaries"][0].shape[1]           # =num_queries*(flat1, two2, three3), 所有set加起来span长度是多少，two有2个set就是4，three有1个set就是3
            n_span = outputs["pred_boundaries"][0].shape[1] // num_queries  # n_span是有几个span，flat是1，two是2，three是3
            tgt_ids = targets["labels"]
            tgt_spans = targets["spans"]
            cost_span = 0.
            for i in range(n_span):    # 第几个span
                cur_span = list(range(i, n_left_right, n_span))
                entity_left = outputs["pred_boundaries"][0][:, cur_span, :].flatten(0, 1)
                entity_right = outputs["pred_boundaries"][1][:, cur_span, :].flatten(0, 1)
                cost_span += -(entity_left[:, tgt_spans[:, i*2]] + entity_right[:, tgt_spans[:, i*2+1]])    # 2是(l,r)长度为2
