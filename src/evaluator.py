...
class Evaluator:
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_entity_bdy:torch.tensor, confidence: float):
        ...
        entity_left = batch_entity_bdy[0].argmax(dim=-1)            # (bsz, q)
...
class DiscontinuousEvaluator:
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_entity_bdy:torch.tensor, confidence: float):
        # flat, two, three
        batch_entity_types_dict, batch_entity_scores_dict, batch_entity_spans_dict, batch_entity_mask_dict = {}, {}, {}, {}
        for key in ['flat', 'two', 'three']:
            batch_size = batch_entity_clf[key].shape[0]
            n_set = batch_entity_clf[key].shape[1]  # 有几个set

            batch_entity_clf_key = batch_entity_clf[key].softmax(dim=-1)
            batch_entity_types = batch_entity_clf_key.argmax(dim=-1) 
            batch_entity_scores = batch_entity_clf_key.max(dim=-1)[0] 
            batch_entity_mask = batch_entity_scores > confidence

            entity_left = batch_entity_bdy[0][key].argmax(dim=-1)
            entity_right = batch_entity_bdy[1][key].argmax(dim=-1) + 1
            if key == 'flat':
                batch_entity_spans = torch.stack([entity_left, entity_right], dim=-1)
                batch_entity_mask = batch_entity_mask * (batch_entity_spans[:,:,0] < batch_entity_spans[:,:,1]) * (batch_entity_types != 0)
            else:
                batch_entity_spans = torch.stack([entity_left, entity_right], dim=-1).view(batch_size, n_set, -1)
                for i in range(0, entity_left.shape[-1], 2):
                    batch_entity_mask = batch_entity_mask * (batch_entity_spans[:,:,i] < batch_entity_spans[:,:,i+1]) * (batch_entity_types != 0)
            
            batch_entity_types_dict[key] = batch_entity_types
            batch_entity_scores_dict[key] = batch_entity_scores
            batch_entity_spans_dict[key] = batch_entity_spans
            batch_entity_mask_dict[key] = batch_entity_mask
            
        for i in range(batch_size):
            sample_pred_entities_i = []
            for key in ['flat', 'two', 'three']:
                entity_mask = batch_entity_mask_dict[key][i]

                entity_types = batch_entity_types_dict[key][i][entity_mask]
                entity_scores = batch_entity_scores_dict[key][i][entity_mask]
                entity_spans = batch_entity_spans_dict[key][i][entity_mask]

                sample_pred_entities = self._convert_pred_entities(entity_types, entity_spans, entity_scores)
                sample_pred_entities = sorted(sample_pred_entities, key=lambda x:x[-1], reverse=True)   # 按照实体分类分数进行排序
                sample_pred_entities = self._remove_duplicate(sample_pred_entities)
                if len(sample_pred_entities) > 0:
                    sample_pred_entities_i.extend(sample_pred_entities)

            self._pred_entities.append(sample_pred_entities_i)
    ...
    def compute_scores(self):
        ...
        self._log("")
        self._log("--- Entities (Discontinuous named entity recognition (NER)) ---")
        self._log("An entity is considered correct if the entity type and span is predicted correctly")
        self._log("")
    ...
    def _convert_pred_entities(self, pred_types: torch.tensor, pred_spans: torch.tensor, pred_scores: torch.tensor):
        ...
        for i in range(pred_types.shape[0]):
            ...
            cls_score = pred_scores[i].item()
            converted_pred = tuple(pred_spans[i].tolist()) + (entity_type, cls_score)
            converted_preds.append(converted_pred)
        return converted_preds
    ...
    def _remove_duplicate(self, entities):
        ...
        for i, can_entity in enumerate(entities):
            ...
            for j, entity in enumerate(non_duplicate_entities):
                find = True
                for i in range(len(can_entity)-3):  # 去掉ent和分数和最后一个pos
                    if can_entity[i] != entity[i] or can_entity[i+1] != entity[i+1]:
                        find = False
            if not find:
                non_duplicate_entities.append(can_entity)
        return non_duplicate_entities
    ...
    def _convert_by_setting(self, gt: List[List[Tuple]], pred: List[List[Tuple]],
        ...
        # either include or remove entity types based on setting
        def convert(t, target=True):
            if not include_entity_types:
                # remove entity type and score for evaluation
                if type(t[0]) == int:  # entity
                    c = t[:-2] + [self._pseudo_entity_type]
                else:  # relation
                    c = [(t[0][0], t[0][1], self._pseudo_entity_type),
                         (t[1][0], t[1][1], self._pseudo_entity_type), t[2]]
            else:
                # 最后一个是分数
                c = list(t) if target else  list(t[:-1])

            if include_score and len(t) > 3 and not target:
                # include prediction scores
                c.append(t[-1])
        ...
        for sample_gt, sample_pred in zip(gt, pred):
            converted_gt.append([convert(t) for t in sample_gt])
            converted_pred.append([convert(t, target=False) for t in sample_pred])
        ...
        for (sample_gt, sample_pred) in zip(gt, pred):
            ...
            for s in union:
                ...
                if s in sample_pred:
                    t = s[-1]
                    if len(s) > 3:
                        pred_discns.append(t.index)
                    pred_flat.append(t.index)
                    types.add(t)
                else:
                    if len(s) > 3:
                        pred_discns.append(0)
                    pred_flat.append(0)
        
        metrics = self._compute_metrics(gt_flat, pred_flat, gt_discns, pred_discns, types, print_results)
        return metrics

    def _compute_metrics(self, gt_all, pred_all, gt_discns, pred_discns, types, print_results: bool = False):
        labels = [t.index for t in types]
        # flat
        per_type = prfs(gt_all, pred_all, labels=labels, average=None)
        micro = prfs(gt_all, pred_all, labels=labels, average='micro')[:-1]
        ...
        total_support = sum(per_type[-1])
        # discontinuous
        per_type_discns = prfs(gt_discns, pred_discns, labels=labels, average=None)
        micro_discns = prfs(gt_discns, pred_discns, labels=labels, average='micro')[:-1]
        macro_discns = prfs(gt_discns, pred_discns, labels=labels, average='macro')[:-1]
        total_support_discns = sum(per_type_discns[-1])
        if print_results:
            self._log("All entities results:")
            self._print_results(per_type, list(micro) + [total_support], list(macro) + [total_support], types)
            self._log("Discontinuous entities results:")
            self._print_results(per_type_discns, list(micro_discns) + [total_support_discns], list(macro_discns) + [total_support_discns], types)
